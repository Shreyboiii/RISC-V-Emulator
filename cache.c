...
uint32_t cache_lookup_dm(struct cache_st *csp, uint64_t addr) {
	...
	b_index = (((addr >> 2) & (csp->block_size - 1) % csp->block_size));
	b_base = addr - (b_index*4);
...
uint32_t cache_lookup_sa(struct cache_st *csp, uint64_t addr) {
    ...
    uint64_t	b_index = (((addr >> 2) & (csp->block_size - 1) % csp->block_size));


    uint64_t b_base = addr - (b_index*4);
    ...
    if (!slot_found) {
        if (slot_invalid) {
            ...
            for (int i = 0; i < 3; i++) {
   	            struct cache_slot_st* v1 = &(csp->slots[set_base + i]);
           		struct cache_slot_st* v2 = &(csp->slots[set_base + i + 1]);
